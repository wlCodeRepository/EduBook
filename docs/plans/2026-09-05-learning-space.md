...
1. 核对目标 Supabase 项目、已有迁移和远端分支；备份受影响预约与旧函数定义，不把业务数据或凭据提交到仓库。
2. 在单事务应用连续课时迁移并检查历史数据、函数权限和约束；失败停止前端发布。
3. 运行验证，发布已核验的 learning-space 分支到 GitHub Pages；无需为发布自动合并主分支。
4. 检查线上资源和可用的真实身份流程，记录发布提交、流水线和未验证边界；临时测试数据单独标识并清理。

发布进度：生产数据库迁移及回滚式业务回归完成；Pages 限制仅 main 可部署，独立分支发布被拒绝。需用户批准合并 main 后继续，不放宽保护规则。详见 docs/deployment/2026-09-05-learning-space-release.md。

后续授权：用户已明确允许合并 main 后上线。先验证 codex/main-backup-20260905 保留旧 main，再同步发布文档、正常合并、从 main 触发 Pages 发布并检查在线资源；不重复数据库迁移，不修改环境保护规则。

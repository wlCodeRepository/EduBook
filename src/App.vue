...
async function submitAuth() {
  ...
  try {
    ...
    if (authView.value === 'login') {
      ...
      if (result.error) throw result.error
      if (result.data.session && result.data.user) { await ensureProfile(result.data.user.id, result.data.user.email || authForm.value.email); showToast('注册成功') } else { signupStep.value = 'code'; showToast('注册验证码已发送，请检查邮箱') }

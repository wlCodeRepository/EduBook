...
async function submitAuth() {
  ...
  try {
    ...
    if (authView.value === 'login') {
      ...
      if (result.error) throw result.error
      authView.value = 'login'; resetStep.value = 'email'; resetCode.value = ''; newPassword.value = ''; showToast('密码已重置，请使用新密码登录')
    }

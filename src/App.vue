...
const adminForm = ref({ username: '', password: '', displayName: '', role: 'STUDENT' as Role, timezone: Intl.DateTimeFormat().resolvedOptions().timeZone || 'Asia/Shanghai' })
const authView = ref<'login' | 'signup' | 'forgot' | 'reset'>('login')
const signupStep = ref<'details' | 'code'>('details')
const resetStep = ref<'email' | 'code' | 'password'>('email')
const resetCode = ref('')
const newPassword = ref('')
const recoveryMode = ref(false)
const signupCooldown = ref(0)
...
function authEmail(identifier: string) { return identifier.includes('@') ? identifier.trim() : `${identifier.trim().toLowerCase()}@accounts.edubook.internal` }
...
function setError(error: unknown) {
  ...
  const errorMap: Record<string, string> = {
    ...
    email_address_invalid: copy.value.authInvalid,
  }
  errorMessage.value = errorMap[code] || (error instanceof Error ? error.message : copy.value.authGeneric)
}

function startCooldown(kind: 'signup' | 'reset') {
  if (cooldownTimer) window.clearInterval(cooldownTimer)
  const target = kind === 'signup' ? signupCooldown : resetCooldown
  target.value = 60
  cooldownTimer = window.setInterval(() => { target.value -= 1; if (target.value <= 0 && cooldownTimer) { window.clearInterval(cooldownTimer); cooldownTimer = null } }, 1000)
}
...
async function loadData() {
  ...
  try {
    ...
    if (profile.value.role === 'ADMIN') {
      ...
      managedUsers.value = usersResponse.data as Profile[]
    }
...
async function submitAuth() {
  ...
  try {
    ...
    if (authView.value === 'login') {
      const result = await supabase.auth.signInWithPassword({ email: authEmail(authForm.value.email), password: authForm.value.password })
      if (result.error) throw result.error
    } else if (authView.value === 'signup' && signupStep.value === 'details') {
      if (signupCooldown.value) return
      ...
      if (result.error) throw result.error
      if (result.data.session && result.data.user) { await ensureProfile(result.data.user.id, result.data.user.email || authForm.value.email); showToast('注册成功') } else { signupStep.value = 'code'; resetCode.value = ''; startCooldown('signup'); showToast('注册验证码已发送，请检查邮箱') }
    } else if (authView.value === 'signup' && signupStep.value === 'code') {
      recoveryMode.value = true
      const result = await supabase.auth.verifyOtp({ email: authForm.value.email, token: resetCode.value.trim(), type: 'email' })
      if (result.error) throw result.error
      if (result.data.user) await ensureProfile(result.data.user.id, result.data.user.email || authForm.value.email)
      recoveryMode.value = false; signupStep.value = 'details'; resetCode.value = ''; showToast('邮箱已验证，注册完成')
  ...
  } catch (error) { if (authView.value === 'forgot' || (authView.value === 'signup' && signupStep.value === 'code')) recoveryMode.value = false; setError(error) } finally { busy.value = false }
}

async function resendResetCode() {
  if (!authForm.value.email || resetCooldown.value) return
  busy.value = true; errorMessage.value = ''
  try { const result = await supabase.auth.resetPasswordForEmail(authForm.value.email); if (result.error) throw result.error; resetCode.value = ''; startCooldown('reset'); showToast('新的重置验证码已发送') } catch (error) { setError(error) } finally { busy.value = false }
}
...
async function createManagedUser() {
  busy.value = true; errorMessage.value = ''
  try {
    const response = await supabase.functions.invoke('admin-create-user', { body: adminForm.value })
    if (response.error) throw response.error
    showToast(language.value === 'en' ? 'Account created. Share the username and password securely.' : '账户已创建，请安全地把账号和密码交给用户。')
    adminForm.value = { username: '', password: '', displayName: '', role: 'STUDENT', timezone: detectedTimezone || 'Asia/Shanghai' }
    await loadData()
...
onUnmounted(() => { unsubscribeAuth?.(); unsubscribeAuth = null; if (cooldownTimer) window.clearInterval(cooldownTimer) })
</script>

<template>
  <div v-if="!session || !profile || recoveryMode" class="auth-shell"><div class="auth-card"><div class="auth-top"><div class="brand"><span class="brand-mark">E</span><span>EduBook</span></div><button class="language-button" @click="toggleLanguage">{{ copy.language }}</button></div><span class="section-kicker">{{ copy.workspace }}</span><h1>{{ authView === 'forgot' ? copy.forgotTitle : authView === 'reset' ? copy.resetTitle : authView === 'login' ? copy.loginTitle : copy.signupTitle }}</h1><p class="auth-intro">{{ authView === 'forgot' || authView === 'reset' ? copy.resetHint : copy.intro }}</p><div v-if="!supabaseConfigured" class="notice error-state">{{ copy.setupMissing }}</div><form @submit.prevent="submitAuth"><template v-if="authView === 'login'"><label>{{ language === 'en' ? 'Username' : '账号名' }}<input v-model="authForm.email" type="text" required autocomplete="username" placeholder="alex.chen" /></label><label>{{ copy.password }}<input v-model="authForm.password" type="password" minlength="8" required autocomplete="current-password" placeholder="••••••••" /></label></template><template v-else-if="authView === 'signup' && signupStep === 'details'"><label>{{ copy.name }}<input v-model="authForm.displayName" required placeholder="Alex" /></label><label>{{ copy.email }}<input v-model="authForm.email" type="email" required placeholder="you@example.com" /></label><label>{{ copy.password }}<input v-model="authForm.password" type="password" minlength="6" required placeholder="••••••••" /></label><label>{{ copy.role }}<select v-model="authForm.role"><option value="STUDENT">{{ copy.student }}</option><option value="TEACHER">{{ copy.teacher }}</option></select></label><label>{{ timezoneAutoDetected ? copy.detectedTimezone : copy.timezone }}<select v-model="authForm.timezone"><option v-for="zone in timezoneOptions" :key="zone" :value="zone">{{ zone }}</option></select></label></template><template v-else-if="authView === 'signup' && signupStep === 'code'"><label>{{ copy.email }}<input :value="authForm.email" type="email" readonly disabled /></label><label>{{ copy.signupCode }}<input v-model="resetCode" inputmode="numeric" autocomplete="one-time-code" pattern="[0-9]{6}" minlength="6" maxlength="6" required placeholder="123456" /></label><p class="otp-hint">{{ copy.signupHint }}</p></template><label v-else-if="authView === 'forgot' && resetStep === 'email'">{{ copy.email }}<input v-model="authForm.email" type="email" required placeholder="you@example.com" /></label><label v-else-if="authView === 'forgot' && resetStep === 'code'">{{ copy.resetCode }}<input v-model="resetCode" inputmode="numeric" autocomplete="one-time-code" pattern="[0-9]{6}" minlength="6" maxlength="6" required placeholder="123456" /></label><label v-else>{{ copy.newPassword }}<input v-model="newPassword" type="password" minlength="6" required placeholder="••••••••" /></label><p v-if="authView === 'forgot' && resetStep === 'code'" class="otp-hint">{{ copy.otpHint }}</p><p v-if="errorMessage" class="form-error">{{ errorMessage }}</p><button class="primary-button full" :disabled="busy || !supabaseConfigured">{{ busy ? copy.processing : authView === 'login' ? copy.login : authView === 'signup' && signupStep === 'details' ? copy.sendSignupCode : authView === 'signup' ? copy.verifySignup : authView === 'forgot' ? resetStep === 'email' ? copy.sendResetCode : resetStep === 'code' ? copy.verifyCode : copy.resetPassword : resetStep === 'email' ? copy.sendResetCode : resetStep === 'code' ? copy.verifyCode : copy.resetPassword }}</button></form><button v-if="authView === 'signup' && signupStep === 'code'" class="text-button auth-switch" :disabled="busy || Boolean(signupCooldown)" @click="resendSignupCode">{{ signupCooldown ? `${signupCooldown}s` : copy.resendCode }}</button><button v-if="authView === 'signup' && signupStep === 'code'" class="text-button auth-switch" :disabled="busy" @click="signupStep = 'details'; resetCode = ''; errorMessage = ''">{{ copy.changeEmail }}</button><button v-if="authView === 'forgot' && resetStep === 'code'" class="text-button auth-switch" :disabled="busy || Boolean(resetCooldown)" @click="resendResetCode">{{ resetCooldown ? `${resetCooldown}s` : copy.sendResetCode }}</button><p class="auth-note">{{ language === 'en' ? 'Accounts are created by an administrator.' : '账户由管理员创建。' }}</p><button v-if="false" class="text-button forgot-link" @click="authView = 'forgot'; errorMessage = ''; resetStep = 'email'">{{ copy.forgotPassword }}</button></div></div>
  <div v-else class="app-shell"><aside class="sidebar" :aria-label="language === 'en' ? 'Main navigation' : '主导航'"><div class="brand"><span class="brand-mark">E</span><span>EduBook</span></div><div class="workspace-label">{{ copy.workspace }}</div><nav class="nav-list"><button class="nav-item" :class="{ active: activeNav === '预约课程' }" @click="activeNav = '预约课程'">□ {{ copy.booking }}</button><button class="nav-item" :class="{ active: activeNav === '我的预约' }" @click="activeNav = '我的预约'">↗ {{ copy.myBookings }} <span class="nav-count">{{ studentBookings.length }}</span></button><template v-if="profile.role === 'ADMIN'"><button class="nav-item" :class="{ active: activeNav === '用户管理' }" @click="activeNav = '用户管理'">＋ {{ language === 'en' ? 'User management' : '用户管理' }} <span class="nav-count">{{ managedUsers.length }}</span></button></template><template v-if="profile.role === 'TEACHER'"><button class="nav-item" :class="{ active: activeNav === '预约管理' }" @click="activeNav = '预约管理'">≡ {{ copy.manage }} <span class="nav-count warm">{{ pendingBookings.length }}</span></button><button class="nav-item" :class="{ active: activeNav === '排期设置' }" @click="activeNav = '排期设置'">＋ {{ copy.schedule }}</button></template></nav><div class="sidebar-bottom"><button class="nav-item">? Help</button><div class="profile-mini"><div class="avatar avatar-user">{{ profile.display_name.slice(0, 2) }}</div><div><strong>{{ profile.display_name }}</strong><small>{{ profile.timezone }}</small></div><button class="signout" @click="signOut">{{ copy.signOut }}</button></div></div></aside>
